...
function ewn_export_user_pdf() {
	...
	$highest_educational_qualification=get_user_meta($user_id,'highest_educational_qualification',true);
	$do_you_own_a_business_=get_user_meta($user_id,'do_you_own_a_business_',true);
	$occupation_job_title=get_user_meta($user_id,'occupation_job_title',true);
	$duration_in_your_current_position=get_user_meta($user_id,'duration_in_your_current_position',true);
	$previous_position=get_user_meta($user_id,'previous_position',true);
	$number_of_years=get_user_meta($user_id,'number_of_years',true);
	$company_name=get_user_meta($user_id,'company_name',true);
	$company_address =get_user_meta($user_id,'company_address',true);
    $company_telephone =get_user_meta($user_id,'company_telephone',true); 
    $company_website =get_user_meta($user_id,'company_website',true); 
    $your_work_email_address =get_user_meta($user_id,'your_work_email_address',true); 
    $business_name =get_user_meta($user_id,'business_name',true);
    $type_of_business =get_user_meta($user_id,'type_of_business',true);
    $capital_invested =get_user_meta($user_id,'capital_invested',true); 
    $year_of_establishment =get_user_meta($user_id,'year_of_establishment',true); 
    $annual_turnover =get_user_meta($user_id,'annual_turnover',true); 
    $number_of_staff_employed =get_user_meta($user_id,'number_of_staff_employed',true); 
    $name_of_company =get_user_meta($user_id,'name_of_company',true); 
    $_number_of_years_in_executive_position =get_user_meta($user_id,'_number_of_years_in_executive_position',true);
    $business_address =get_user_meta($user_id,'business_address',true);
    $business_telephone =get_user_meta($user_id,'business_telephone',true);
    $business_website =get_user_meta($user_id,'business_website',true);
    $business_email_address =get_user_meta($user_id,'business_email_address',true); 
    $industry = get_user_meta($user_id, 'industry', true);
    $how_did_you_hear_of_ewn_ =get_user_meta($user_id,'how_did_you_hear_of_ewn_',true);
    $name_membership_number =get_user_meta($user_id,'name_membership_number',true);
    $specify =get_user_meta($user_id,'specify',true); 
	$skills =get_user_meta($user_id,'list_your_skills_areas_of_competence',true); 
	$briefly =get_user_meta($user_id,'briefly_tell_us_why_you_would_like_to_be_a_member_of_ewn_and_what_your_expectations_are_for_joining_this_network',true); 
...
$pdf->SetFont('Times','',12);
   ...
   $pdf->SetFont('Times','',12);
   $pdf->Cell(0,10,'Business Name: ',0,1,);
   ...
   $pdf->Cell(0,10,'Number of years in executive position: ',0,1,);
